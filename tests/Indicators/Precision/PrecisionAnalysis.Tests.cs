...
public class BoundaryTests : TestBase
{
    ...
    public void RSI_WithMonotonicallyIncreasingPrices_StaysBelow100()
    {
        ...
        // Analyze
        AnalyzeBounds("RSI Series (increasing)", seriesResults, x => x.Rsi, 0, 100);
        AnalyzeBounds("RSI Buffer (increasing)", bufferResults, x => x.Rsi, 0, 100);
        AnalyzeBounds("RSI Stream (increasing)", streamResults, x => x.Rsi, 0, 100);
    }
    ...
    public void RSI_WithMonotonicallyDecreasingPrices_StaysAbove0()
    {
        ...
        // Analyze
        AnalyzeBounds("RSI Series (decreasing)", seriesResults, x => x.Rsi, 0, 100);
        AnalyzeBounds("RSI Buffer (decreasing)", bufferResults, x => x.Rsi, 0, 100);
        AnalyzeBounds("RSI Stream (decreasing)", streamResults, x => x.Rsi, 0, 100);
    }
    ...
    public void Stoch_WithCloseEqualsHigh_StaysBelow100()
    {
        ...
        AnalyzeBounds("Stoch Buffer (close=high)", bufferResults, x => x.Oscillator, 0, 100);
        AnalyzeBounds("Stoch Stream (close=high)", streamResults, x => x.Oscillator, 0, 100);
    }
    ...
    public void Stoch_WithCloseEqualsLow_StaysAbove0()
    {
        ...
        AnalyzeBounds("Stoch Buffer (close=low)", bufferResults, x => x.Oscillator, 0, 100);
        AnalyzeBounds("Stoch Stream (close=low)", streamResults, x => x.Oscillator, 0, 100);
    }
    ...
    public void StochRSI_WithMonotonicallyIncreasingPrices_StaysWithinBounds()
    {
        ...
        // Analyze
        AnalyzeBounds("StochRSI Series (increasing)", seriesResults, x => x.StochRsi, 0, 100);
        AnalyzeBounds("StochRSI Buffer (increasing)", bufferResults, x => x.StochRsi, 0, 100);
        AnalyzeBounds("StochRSI Stream (increasing)", streamResults, x => x.StochRsi, 0, 100);
    }
    ...
    public void CMO_WithAlternatingPrices_StaysWithinBounds()
    {
        ...
        // Analyze
        AnalyzeBounds("CMO Series (alternating)", seriesResults, x => x.Cmo, -100, 100);
        AnalyzeBounds("CMO Buffer (alternating)", bufferResults, x => x.Cmo, -100, 100);
        AnalyzeBounds("CMO Stream (alternating)", streamResults, x => x.Cmo, -100, 100);
    }
    ...
    public void MFI_WithMonotonicallyIncreasingPrices_StaysWithinBounds()
    {
        ...
        // Analyze
        AnalyzeBounds("MFI Series (increasing)", seriesResults, x => x.Mfi, 0, 100);
        AnalyzeBounds("MFI Buffer (increasing)", bufferResults, x => x.Mfi, 0, 100);
        AnalyzeBounds("MFI Stream (increasing)", streamResults, x => x.Mfi, 0, 100);
    }
    ...
    public void WilliamsR_WithCloseEqualsHigh_StaysWithinBounds()
    {
        ...
        // Analyze
        AnalyzeBounds("WilliamsR Series (close=high)", seriesResults, x => x.WilliamsR, -100, 0);
        AnalyzeBounds("WilliamsR Buffer (close=high)", bufferResults, x => x.WilliamsR, -100, 0);
        AnalyzeBounds("WilliamsR Stream (close=high)", streamResults, x => x.WilliamsR, -100, 0);
    }
    ...
    public void TSI_WithMonotonicallyIncreasingPrices_StaysWithinBounds()
    {
        ...
        // Analyze
        AnalyzeBounds("TSI Series (increasing)", seriesResults, x => x.Tsi, -100, 100);
        AnalyzeBounds("TSI Buffer (increasing)", bufferResults, x => x.Tsi, -100, 100);
        AnalyzeBounds("TSI Stream (increasing)", streamResults, x => x.Tsi, -100, 100);
    }
