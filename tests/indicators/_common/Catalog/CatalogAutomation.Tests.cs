...
public class CatalogAutomationTests : TestBase
{
    ...
    public void CatalogExecutionShouldWorkForKnownIndicators()
    {
        ...
        foreach (string uiid in testIndicators)
        {
            IndicatorListing? listing = catalog.FirstOrDefault(l => l.Uiid == uiid && l.Style == Style.Series);
            ...
            try
            {
                ...
                IReadOnlyList<IReusable> catalogResults = listing.Execute<IReusable>(quotes);
    ...
    {
        // Arrange
        IndicatorListing emaListing = IndicatorCatalog.Catalog.First(l => l.Uiid == "EMA" && l.Style == Style.Series);
    ...
    public void CatalogStyleCountsShouldMatchExpected()
    {
        // Arrange & Act
        IReadOnlyList<IndicatorListing> catalog = IndicatorCatalog.Catalog;
        Dictionary<Style, int> styleCounts = catalog
            .GroupBy(l => l.Style)
