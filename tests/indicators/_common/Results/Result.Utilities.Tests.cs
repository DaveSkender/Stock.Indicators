...
public class Results : TestBase
{
    ...
    public void ToTuple()
    {
        ...
        Assert.HasCount(5, chainableTuple.Where(static x => !double.IsNaN(x.Value)));
        Assert.HasCount(2, chainableTuple.Where(static x => double.IsNaN(x.Value)));
        ...
        Assert.HasCount(5, cnaNresults.Where(static x => !double.IsNaN(x.Value)));
        Assert.HasCount(2, cnaNresults.Where(static x => double.IsNaN(x.Value)));

        // with NaN option, no pruning
        Collection<(DateTime Date, double Value)> nanResults = baseline
            .ToTupleNaN();

        Assert.HasCount(4, nanResults.Where(static x => double.IsNaN(x.Value)));
