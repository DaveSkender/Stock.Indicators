...
public class PivotsTests : TestBase
{
    ...
    public void PivotsSeriesFromCatalogMatchesDirectCall()
    {
        ...
        {
            directResults = quotes.ToPivots(parameters[0]);
        }
        else if (parameters.Length == 2)
        {
            directResults = quotes.ToPivots(parameters[0], parameters[1]);
        }
        else if (parameters.Length == 3)
        {
            directResults = quotes.ToPivots(parameters[0], parameters[1], parameters[2]);
        }
        else
        {
