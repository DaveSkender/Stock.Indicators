...
public class Renko : TestBase
{
    ...
    public void StandardHighLow()
    {
        ...
        Assert.AreEqual(100801672m, NullMath.Round(r25.Volume, 0));
        ...
        Assert.AreEqual(51999637m, NullMath.Round(last.Volume, 0));
        Assert.IsTrue(last.IsUp);
    }
    ...
    public void Atr()
    {
        ...
        RenkoResult last = results.LastOrDefault();
        Assert.AreEqual(237.3990m, NullMath.Round(last.Open, 4));
        Assert.AreEqual(246.73m, NullMath.Round(last.High, 4));
        Assert.AreEqual(229.42m, NullMath.Round(last.Low, 4));
        Assert.AreEqual(243.5487m, NullMath.Round(last.Close, 4));
        Assert.AreEqual(715446448m, NullMath.Round(last.Volume, 0));
        Assert.IsTrue(last.IsUp);
    }
