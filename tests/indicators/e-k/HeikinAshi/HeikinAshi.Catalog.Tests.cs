using System.Reflection;

namespace Catalog;

/// <summary>
/// Test class for HeikinAshi catalog functionality.
/// Validates that catalog-based execution produces identical results to direct calls.
/// </summary>
[TestClass]
public class HeikinAshiTests : TestBase
{
    [TestMethod]
    public void HeikinAshiSeriesFromCatalogMatchesDirectCall()
    {
        // Arrange
        IReadOnlyList<Quote> quotes = Quotes;
        IndicatorListing listing = HeikinAshi.SeriesListing;
        listing.Should().NotBeNull();

        // Act - Call using catalog metadata
        IReadOnlyList<HeikinAshiResult> catalogResults = listing.Execute<HeikinAshiResult>(quotes);

        // Act - Direct call for comparison using default parameters
        var parameters = listing.Parameters?.Where(p => p.IsRequired && p.DefaultValue != null)
            .Select(p => p.DefaultValue).ToArray() ?? new object[0];
        
        IReadOnlyList<HeikinAshiResult> directResults;
        if (parameters.Length == 0)
        {
            directResults = quotes.ToHeikinAshi();
        }
        else if (parameters.Length == 1)
        {
            directResults = quotes.ToHeikinAshi(parameters[0]);
        }
        else if (parameters.Length == 2)
        {
            directResults = quotes.ToHeikinAshi(parameters[0], parameters[1]);
        }
        else if (parameters.Length == 3)
        {
            directResults = quotes.ToHeikinAshi(parameters[0], parameters[1], parameters[2]);
        }
        else
        {
            // Use reflection for complex parameter cases
            var method = typeof(HeikinAshi).GetMethod("ToHeikinAshi", BindingFlags.Public | BindingFlags.Static);
            method.Should().NotBeNull("Method ToHeikinAshi should exist");
            directResults = (IReadOnlyList<HeikinAshiResult>)method!.Invoke(null, 
                new object[] { quotes }.Concat(parameters).ToArray());
        }

        // Assert - Results should be identical
        catalogResults.Should().BeEquivalentTo(directResults);
    }
}
