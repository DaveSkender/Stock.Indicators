...
public class IchimokuTests : TestBase
{
    ...
    public void IchimokuSeriesFromCatalogMatchesDirectCall()
    {
        ...
        {
            directResults = quotes.ToIchimoku((int)parameters[0]);
        }
        else if (parameters.Length == 2)
        {
            directResults = quotes.ToIchimoku((int)parameters[0], (int)parameters[1]);
        }
        else if (parameters.Length == 3)
        {
            directResults = quotes.ToIchimoku((int)parameters[0], (int)parameters[1], (int)parameters[2]);
        }
        else
        {
