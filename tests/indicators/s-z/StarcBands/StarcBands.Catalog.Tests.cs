...
public class StarcBandsTests : TestBase
{
    ...
    public void StarcBandsSeriesFromCatalogMatchesDirectCall()
    {
        ...
        {
            directResults = quotes.ToStarcBands(parameters[0]);
        }
        else if (parameters.Length == 2)
        {
            directResults = quotes.ToStarcBands(parameters[0], parameters[1]);
        }
        else if (parameters.Length == 3)
        {
            directResults = quotes.ToStarcBands(parameters[0], parameters[1], parameters[2]);
        }
        else
        {
