...
public class UltimateTests : TestBase
{
    ...
    public void UltimateSeriesFromCatalogMatchesDirectCall()
    {
        ...
        {
            directResults = quotes.ToUltimate(parameters[0]);
        }
        else if (parameters.Length == 2)
        {
            directResults = quotes.ToUltimate(parameters[0], parameters[1]);
        }
        else if (parameters.Length == 3)
        {
            directResults = quotes.ToUltimate(parameters[0], parameters[1], parameters[2]);
        }
        else
        {
