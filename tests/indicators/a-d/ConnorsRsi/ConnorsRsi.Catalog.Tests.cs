...
public class ConnorsRsiTests : TestBase
{
    ...
    public void ConnorsRsiSeriesFromCatalogMatchesDirectCall()
    {
        ...
        {
            directResults = quotes.ToConnorsRsi(parameters[0]);
        }
        else if (parameters.Length == 2)
        {
            directResults = quotes.ToConnorsRsi(parameters[0], parameters[1]);
        }
        else if (parameters.Length == 3)
        {
            directResults = quotes.ToConnorsRsi(parameters[0], parameters[1], parameters[2]);
        }
        else
        {
