...
public class ConnorsRsiTests : TestBase
{
    ...
    public void ConnorsRsiSeriesListing()
    {
        ...
        IndicatorParam streakPeriodsParam1 = listing.Parameters.SingleOrDefault(p => p.ParameterName == "streakPeriods");
        streakPeriodsParam1.Should().NotBeNull();
        IndicatorParam rankPeriodsParam2 = listing.Parameters.SingleOrDefault(p => p.ParameterName == "rankPeriods");
        rankPeriodsParam2.Should().NotBeNull();

        listing.Results.Should().NotBeNull();
        listing.Results.Should().HaveCount(5);

        IndicatorResult streakResult = listing.Results.SingleOrDefault(r => r.DataName == "Streak");
        streakResult.Should().NotBeNull();
        streakResult?.DisplayName.Should().Be("Streak");
        streakResult.IsReusable.Should().Be(false);
        IndicatorResult rsiResult1 = listing.Results.SingleOrDefault(r => r.DataName == "Rsi");
        rsiResult1.Should().NotBeNull();
        rsiResult1?.DisplayName.Should().Be("RSI");
        rsiResult1.IsReusable.Should().Be(false);
        IndicatorResult rsistreakResult2 = listing.Results.SingleOrDefault(r => r.DataName == "RsiStreak");
        ...
        IndicatorResult percentrankResult3 = listing.Results.SingleOrDefault(r => r.DataName == "PercentRank");
        ...
        IndicatorResult connorsrsiResult4 = listing.Results.SingleOrDefault(r => r.DataName == "ConnorsRsi");
