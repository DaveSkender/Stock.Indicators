...
| `kijunPeriods` | int | Number of periods (`K`) in the shorter Kijun-sen midpoint evaluation.  It also sets the Senkau and Chikou offsets.  Must be greater than 0.  Default is 26.
...
| `offsetPeriods` | int | Optional.  Number of periods to offset both `Senkou` and `Chikou` spans.  Must be >= 0.  Default is `kijunPeriods`.
| `offsetPeriods` | int | Optional.  Number of periods to offset both `Senkou` and `Chikou` spans.  Must be >= 0.  Default is `kijunPeriods`.
| `senkouOffset` | int | Optional.  Number of periods to offset the `Senkou` span.  Must be >= 0.  Default is `kijunPeriods`.
| `chikouOffset` | int | Optional.  Number of periods to offset the `Chikou` span.  Must be >= 0.  Default is `kijunPeriods`.

See overloads usage above to determine which parameters are relevant for each.

### Historical quotes requirements

You must have at least the greater of `T`,`K`, or `S` periods of `quotes`; though, given the leading and lagging nature, we recommend notably more.
...
- The first `T-1`, `K-1`, and `S-1` periods will have various `null` values since there's not enough data to calculate.
