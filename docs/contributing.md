...
- Historical Stock Quotes are automatically added to unit test methods.  A `TestData.xlsx` Excel file is included in the `data` folder that is an exact copy of what is used in the unit tests.  Use this for your manual calculations to ensure that it is correct.  Do not commit changes to this Excel file.
...
Running the `Tests.Performance` console application in `Release` mode will produce performance data that we periodically include in the repo documentation.  You can find the latest results [here]({{site.baseurl}}/performance).
