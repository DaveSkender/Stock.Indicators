...
<!-- categories -->
{% for c in site.data.categories %}
<h2 id="{{c.type}}">{{c.name}}</h2>
{% assign indicators = site.indicators | where: "type", c.type %}
{% assign aliases = site.data.aliases | where: "type", c.type %}
...
<h3 id="{{s.type}}">{{s.name}}</h3>
{% assign indicators = site.indicators | where: "type", s.type %}
{% assign aliases = site.data.aliases | where: "type", s.type %}
{% assign all = indicators | concat: aliases | sort: "title" %}
<ul>
  {% for i in all %}
  <li><a href="{{i.permalink}}">{{i.title}}</a></li>
