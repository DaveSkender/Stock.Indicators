...
**Update (2026-06-13).** Since the §E/§L closeout, the full §L v3.0-readiness batch merged to `v3` — root-hub mutation guard + `Remove(IQuote/ITick)` (#2059), observer-set synchronization + `Reinitialize` gap close (#2069, SR007/SR006c), `Snapshot()` (#2061), the concurrency safety-net suite (#2070), aggregator/deep-chain rollback coverage (#2071), and BufferList bounded-value parity (#2072). `IDisposable` was built then deliberately withdrawn (#2060) and deferred to [#2068](https://github.com/DaveSkender/Stock.Indicators/issues/2068)/[#2078](https://github.com/DaveSkender/Stock.Indicators/issues/2078). A **memory/GC + API-surface hardening pass landed (PR #2073, 2026-06-12)** that closes several §E follow-ups and introduces a new internal `RollbackRing<TState>` primitive (`src/_common/StreamHub/RollbackRing.cs`) — see the §E follow-up updates below and the ARCH-V31-11 correctness note. The CI performance workflows were repaired (PR #2082); **RG001's core baselines (Series/Buffer/Stream) were then refreshed and committed in PR #2087** — regenerated on the reference host with `--job short`, which also surfaced and fixed a `test-performance.yml` config/timeout defect (it ran the slow DefaultConfig path that now exceeds its 90-min timeout). See §A RG001 for detail. A "v3 use-case/ergonomics assessment" filed seven new tracking issues ([#2076](https://github.com/DaveSkender/Stock.Indicators/issues/2076)–[#2081](https://github.com/DaveSkender/Stock.Indicators/issues/2081), plus [#2083](https://github.com/DaveSkender/Stock.Indicators/issues/2083)); they are cross-referenced into the v3.1+ backlog below. **§L is therefore cleared except the three pending maintainer decisions (RG004, SR001, ARCH-V31-11); the remaining v3.0 gates are §A (release runtime) and §K (release mechanics).**
...
| ------ | ----- | ---------------- |
| Release gates | ~~RG001 baseline refresh~~ ✅ core done (#2087), RG002 community feedback window, RG004 Quote→Bar decision | maintainer asyncs |
...
- [~] **RG001 — Refresh performance baselines** (core styles done; PR #2087). The three core baselines — `Performance.{Series,Buffer,Stream}Indicators-report-full.json` — were regenerated and committed in **PR #2087**, replacing numbers last refreshed 2025-12-30 (#1816). They were stale on multiple axes: pre-#2073 hot paths, the Series `ToXxx`→`ToXxxBatch` benchmark rename (zero method overlap with the old report), and the removal of the non-streamable `BetaHub`/`CorrelationHub`/`PrsHub` stream benchmarks (83→80).
  - **Methodology correction (important).** The existing baselines are **not** CI-generated — they were produced **locally on the reference host** (13th-gen Intel i9-13900H, Windows 11 build 26200) with **`--job short`** (ShortRun), and the numbers are hardware-sensitive, so they must be regenerated on that same consistent machine, not on an `ubuntu-latest` runner. PR #2087 was generated there; `tools/performance/baselines/README.md` now documents this. The benchmark classes carry `[ShortRunJob, WarmupCount(5), IterationCount(5)]` attributes, so the new baselines are `ShortRun(5/1/5)` vs the prior baselines' `(3/1/3)` — more iterations, tighter intervals, means still comparable. (The job comes from the class attributes, not a CLI flag — `dotnet run` with or without `--job short` yields the same ShortRun.)
  - **Workflow timeout fix (PR #2087).** The perf workflows' `timeout-minutes: 90` was sized for the pre-#2082 no-op — BenchmarkDotNet couldn't build its harness before that fix, so runs failed fast (the "349 s success"). Post-#2082 the benchmarks actually execute, and the genuine ShortRun suites take multiple hours on a 2-core hosted runner, exceeding 90 min. Raised both `test-performance.yml` and `test-performance-comparison.yml` to 360. **No config/job change was needed**: the core classes are `[ShortRunJob, WarmupCount(5), IterationCount(5)]` and `StyleComparison` is `[ShortRunJob]`, so the no-arg/`--filter` runs are already ShortRun via the attributes (verified empirically — the no-arg path reports `ShortRun(5/1/5)`). An earlier draft of this PR mis-attributed the slowness to a DefaultConfig(15/15) mismatch and added a redundant `--job short`; that was reverted.
  - **#2085 SMA caveat.** PR #2087's baselines were measured on v3 @ #2073, just before #2085 (SMA `Queue<double>` buffer carried into the Series and Stream styles) merged. The benchmark *methodology* PRs that merged alongside it (#2089/#2091/#2092/#2093) touched only `Perf.ManualTestDirect.cs` (the manual harness), so the core baselines stay methodologically valid; #2085's perf delta is isolated to two rows — Series `ToSmaBatch` and Stream `SmaHub` (~12–14% faster per the #2085 report). Those two read slightly pessimistic vs current code, which is harmless for `detect-regressions.ps1` (it only flags slowdowns). Refresh them at the next full baseline run.
  - **`void`→`object` Series benchmark fix (PR #2087).** The per-class `SeriesIndicators` benchmarks returned `void` (unlike Buffer/Stream, which return their result), risking dead-code elimination and making cross-style reads from these per-class files unreliable — a controlled `StyleComparison` run showed Series fastest for EMA/SMA/MACD/ADX, opposite to the void per-class numbers. They now return `object` so BenchmarkDotNet consumes the result. The committed Series baseline was captured just before this change and is **provisional**; regenerate it (and `StyleComparison`) with the README command on the reference host. For cross-style "which is fastest" questions use `StyleComparison`, not the per-class files.
  - **Remaining (optional, not core RG001).** `StyleComparison` (now refreshable — #2085 has merged), `Utility`, `UtilityStdDev`, and `StreamExternal` baselines were not refreshed here. `tools/performance/baselines/README.md` now carries the full regeneration + copy commands for every class (StyleComparison is a ~2h run). Refresh on the reference host when convenient; optionally tag a `baseline-v3.0.0.json` snapshot at the stable tag.
...
- [ ] **ARCH-V31-1 — Retire `BaseProvider<T>`, introduce `StreamSource<T>`** (4–6 hours).
  ...
  - **Progress (PR #2073):** `BaseProvider<T>` was downgraded from `public` to `internal` (`BaseProvider.cs:26`), shrinking the public surface ahead of the refactor; the inert-root workaround itself still stands.
...
- [ ] **ARCH-V31-6 — `IAsyncEnumerable<T>` and `Channels` adapters** (3–4 hours) — the `Channel`-based feeder half is filed as [#2077](https://github.com/DaveSkender/Stock.Indicators/issues/2077) (`quoteHub.CreateFeed()` / `WriteAsync`, single-consumer drain, bounded-channel backpressure).
...
- [ ] **ARCH-V31-7a — `Sma.Increment(Queue<double>)` overload + BufferList sweep** (4–6 hours) — tracked as [#2083](https://github.com/DaveSkender/Stock.Indicators/issues/2083); **PR #2085 is open** and broadens the scope to carry the `Queue<double>` buffer into **all three** SMA styles (the StaticSeries variant too — overlaps S001), reporting ~14% Series / ~12% Stream speedups. PR #2085 still has two open checklist questions (perf-diff confirmation; why Series and Buffer don't fully converge) — review and resolve before merge.
  - Add a `Queue<double>` overload to `Sma.Utilities.cs` (formalize the existing `Average(Queue<double>)` extension as `Increment` to align naming with other kernels). Migrate ~12–15 BufferLists currently inlining `sum / count` over their queue: Tema, Trix, T3, MaEnvelopes, BollingerBands, StdDev, Cmf, Cci, Mfi, Smma SMA-seed, ConnorsRsi, others. Math algebraically identical; pin with `IsExactly` against series oracle. (Note: #2073 already replaced LINQ `Average()`/`Sum()` over `Queue<double>` with manual in-order sums in several of these hot paths — Stoch, BollingerBands, Cci, StdDev, Slope, RocWb — so the remaining work is kernel *consolidation*, not the per-call allocation fix.)
...
- [ ] **ARCH-V31-11 — Prune-stable rollback: framework state snapshot/restore** (large; the SR003 real fix — dedicated multi-PR effort, **do not bolt onto a hardening pass**). **⚠️ PENDING v3.0 DECISION (see §L)** — pull into v3.0 (engine bit-exact under every cache config) or ship the documented edge-config limitation.
  ...
  - **Foundation already landed (PR #2073).** Part (b) of the design below now exists in concrete form: `RollbackRing<TState>` (`src/_common/StreamHub/RollbackRing.cs`) plus per-hub `SnapshotState`/`RestoreState`, retrofitted to Adx, Rsi, Kvo, ParabolicSar, SuperTrend. This removes the post-prune drift for **near-tail** corrections on those hubs (the ring holds the bit-exact live state for the last 32 distinct timestamps). The remaining ARCH-V31-11 work is therefore narrower than originally scoped: (a) the pre-prune boundary capture for **deep** (beyond-ring) rollbacks, and (c) extending the ring/snapshot pattern to the rest of the stateful hubs (EMA family, ATR, SMMA, OBV/ADL, …).
  - **Design.** (a) Capture the pruned provider items at prune time — add a pre-prune hook, or pass the pruned items into a new `PruneState(DateTime, IReadOnlyList<TIn>)` overload, so a hub can fold them into a preserved boundary snapshot (do **not** change the public `IStreamObserver.OnPrune` signature). (b) ~~Add `protected virtual SnapshotState()` / `RestoreState(...)`~~ **— shipped as `RollbackRing<TState>` in #2073 for 5 hubs; generalize the adoption.** (c) Adopt across all stateful hubs. (d) Pin with `IsExactly` oracle tests and **invert the divergence assertions in `StreamHub.RollbackAfterPrune.Tests.cs` to exact-equality** as the acceptance gate.
...
- [ ] **ARCH-V31-12 — Guard mutation on subscribed (non-root) hubs** (2–3 hours).
  ...
  - Companion (same root-only constraint): a convenience `Remove(IQuote)` on the root `QuoteHub`/`TickHub` (find-by-timestamp → `RemoveAt`/`RemoveRange`; no-op-or-throw if absent), from the §E SR011 doc discussion. Decide whether to ship the two together. **Shipped in PR #2059** (`Remove(IQuote/ITick)` + root-only mutation guard).

### Production ergonomics — v3 use-case assessment (2026-06-10)

New tracking issues from a post-#2073 production use-case/ergonomics review. None block v3.0; all are additive (non-breaking) so they can land in any v3.x minor. Prioritized by the assessment's severity.

- [ ] **PROD-1 — Warm-start state capture/restore** — [#2076](https://github.com/DaveSkender/Stock.Indicators/issues/2076) (High). `hub.CaptureState()` / `RestoreState(snapshot)` (and the BufferList analog) so a 24/7 service can rehydrate stateful indicators across process restarts without replaying full quote history. The `RollbackRing<TState>` snapshot pattern from #2073 is the natural foundation; needs a versioned (de)serialization contract that stays bit-exact on restore and resyncs cleanly with the provider (shares the `Reinitialize` concern). Distinct from ARCH-V31-10 (external *cache* storage) — this persists *indicator state*, not the result cache.
- [ ] **PROD-2 — Async feed bridging helper** — [#2077](https://github.com/DaveSkender/Stock.Indicators/issues/2077) (Medium-High). Same item as ARCH-V31-6's `Channel`-feeder half; cross-referenced there.
- [ ] **PROD-3 — Hub lifecycle / multi-symbol management** — [#2078](https://github.com/DaveSkender/Stock.Indicators/issues/2078) (Medium-High). Superset of SR017/[#2068](https://github.com/DaveSkender/Stock.Indicators/issues/2068): `IDisposable` plus a keyed create-or-get hub-set for the symbol-fleet case (thousands of hubs) and a DI/`IHostedService` integration pattern. Mid-chain disposal must detach without tearing down siblings.
- [ ] **PROD-4 — StreamHub observability counters** — [#2079](https://github.com/DaveSkender/Stock.Indicators/issues/2079) (Medium). Allocation-free `RebuildCount` / `PruneCount` / `DuplicatesSuppressed` / `LastRebuildDepth` (today only `IsFaulted`/`OverflowException` surface), optionally via `System.Diagnostics.Metrics.Meter`, plus a `ToDiagnosticsString()` triage summary. No existing plan entry.
- [ ] **PROD-5 — Opt-in intra-bar notification conflation for aggregator hubs** — [#2080](https://github.com/DaveSkender/Stock.Indicators/issues/2080) (Medium). A `BinarySettings`/ctor flag so `QuoteAggregatorHub`/`TickAggregatorHub` notify downstream only on bar **close** (forming bar still queryable via `Results`/`Snapshot()`), cutting ~N× downstream work for N ticks/bar for bar-close consumers. Default unchanged; reopened closed bars must still notify. Related to RES-V32-3 (Pine-style forming-bar fast path) but on the aggregator's *fan-out* side.
...
- [~] **SR009 — Amortize `PruneCache` / `PruneList` O(MaxCacheSize)-per-tick shift** (partially shipped). **StreamHub side done in PR #2073**: `OnPrune` replaced its per-item `RemoveAt(0)` loop with a single counted `RemoveRange` (was O(n²) on multi-item provider prunes). **Still open**: the steady-state per-tick single-item left-shift on both `StreamHub.PruneCache` and `src/_common/BufferLists/BufferList.cs:118` (`RemoveRange(0, overflow)` with `overflow == 1` once at cap is still an O(MaxCacheSize) memmove/tick), and the ring-buffer generalization (generalize `CircularDoubleBuffer` to `TOut`; one base-class change covers all 79 BufferLists). #2073 deliberately left HtTrendline/Mama state-list pruning as-is (ring-buffer conversion judged too risky for a hardening pass). Overlaps ARCH-V31-2 cache-routing.
- [x] **SR010 — Cache the observer-notify snapshot** (2 hours). *(PR #2073 — the subscriber snapshot array is now cached and invalidated only on Subscribe/Unsubscribe/EndTransmission instead of being re-allocated on every Add/Rebuild/Prune/Error notification.)*
...
- [ ] **SR008e — Enforce the BufferList chronological-input precondition** (maintainer-gated; 2–4 hours) — tracked as [#2081](https://github.com/DaveSkender/Stock.Indicators/issues/2081) (filed as opt-in `ThrowOnOutOfOrder` strict mode). Code half of §E SR008 (documented + pinned in PR #2054). A guard in `BufferList.AddInternal` (`item.Timestamp < _internalList[^1].Timestamp` → throw) is viable: an experiment broke **exactly one** indicators-project test (`Pmo.CustomBuffer_OverMaxListSize_AutoAdjustsListAndBuffers`, which itself re-feeds `Quotes.TakeLast(50)` out of order). Before adopting: confirm blast radius across the integration + public-api test projects and consumer impact (a new exception across all 79 BufferLists is a breaking change at stable), decide whether duplicates (`==`) are also rejected, settle the exception type/message, and update `BufferList.OrderingContract.Tests.cs` (which currently pins the *unenforced* behavior) plus the Pmo test.
...
- [ ] **SR017 — Chain lifecycle ergonomics** (3–4 hours) — tracked as [#2068](https://github.com/DaveSkender/Stock.Indicators/issues/2068) and (expanded with multi-symbol fleet management) [#2078](https://github.com/DaveSkender/Stock.Indicators/issues/2078); `IDisposable` was built in PR #2060 then withdrawn (hubs hold no unmanaged resources; `EndTransmission`/`Unsubscribe` + GC sufficient). No `Dispose`/`DisposeChain` on `StreamHub` (it does not implement `IDisposable`); teardown is per-hub via `EndTransmission`/`Unsubscribe` and undiscoverable. Document the `EndTransmission`→`OnCompleted` teardown sequence in `stream.md` and consider implementing `IDisposable`. Surface the `Reinitialize` caveat in xmldoc.
...
- Minor cleanups (Low; batch when next touching the files). **Cleared in PR #2073**: `Add(IEnumerable)` now skips the `OrderBy` allocation on the already-chronological fast path, and the per-rollback delegate alloc is gone (`Cache.RemoveAll(closure)` → binary-search `IndexGte` + `RemoveRange`). **Still open**: `OverflowCount` not reset alongside `LastItem` in `RemoveRange` (replay self-heals; `StreamHub.cs:190`); standalone same-timestamp replacement leaves base `LastItem` stale → one extra rebuild cascade on an idempotent re-send (`Quote.StreamHub.cs:135`); `RemoveRange(DateTime)` is the lone cache mutator with no `CacheLock` (defense-in-depth under the single-writer contract; `StreamHub.cs:175`); `_isRebuilding` is a non-volatile instance bool (add an "only read/written under CacheLock" invariant comment; `:23`); `stream.md:128` "treat as thread-safe (… RemoveRange …)" wording over-promises versus the documented single-writer model.
