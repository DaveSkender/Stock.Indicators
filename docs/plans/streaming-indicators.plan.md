...
- [~] **RG001 — Refresh performance baselines** (core styles done; PR #2087). The three core baselines — `Performance.{Series,Buffer,Stream}Indicators-report-full.json` — were regenerated and committed in **PR #2087**, replacing numbers last refreshed 2025-12-30 (#1816). They were stale on multiple axes: pre-#2073 hot paths, the Series `ToXxx`→`ToXxxBatch` benchmark rename (zero method overlap with the old report), and the removal of the non-streamable `BetaHub`/`CorrelationHub`/`PrsHub` stream benchmarks (83→80).
  - **Methodology correction (important).** The existing baselines are **not** CI-generated — they were produced **locally on the reference host** (13th-gen Intel i9-13900H, Windows 11 build 26200) with **`--job short`** (ShortRun), and the numbers are hardware-sensitive, so they must be regenerated on that same consistent machine, not on an `ubuntu-latest` runner. PR #2087 was generated there; `tools/performance/baselines/README.md` now documents this. The current `--job short` CLI resolves to `ShortRun(5/1/5)` vs the prior `(3/1/3)` — more iterations, tighter intervals, means still comparable.
  - **Workflow defect found + fixed (PR #2087).** `test-performance.yml` ran the no-arg `dotnet run -c Release` path, which produces **DefaultConfig** (15/15), *not* the ShortRun the baselines use; post-#2082 that real full suite runs **>3 hours and exceeds its own `timeout-minutes: 90`** (the pre-#2082 "349 s success" was a no-op). Fixed to pass `--job short` with an explicit class filter and a raised timeout. (`test-performance-comparison.yml` has the same DefaultConfig-without-`--job-short` shape for `StyleComparison`; left as a separate follow-up.)
