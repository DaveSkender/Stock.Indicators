...
- [x] **SR017 — `IDisposable` / chain teardown on `StreamHub`** (3–4 hours). Implement `IDisposable` (+ a `DisposeChain` helper) so teardown is discoverable; document the `EndTransmission`→`OnCompleted` sequence in `stream.md`; surface the `Reinitialize` caveat in xmldoc. Additive, but sets the canonical teardown in the stable surface. *(PR #2060)*
...
- [x] **SR007 — Guard `_observers` against Subscribe-during-stream races** (2–3 hours). `StreamHub.Observable.cs:40,51,95` mutate `_observers` unsynchronized vs the notify-loop enumeration; constructing a downstream hub concurrent with `Add` is a real race. Guard `_observers` (snapshot-on-mutate or lock). Pairs with TC-V31-1. *(PR #2062)*
- [x] **SR006c — Close the `Reinitialize` rebuild→subscribe gap** (part of T205 / `StreamHub.cs:233`). The gap can drop high-frequency data; subscribe-before/atomic-with rebuild, or re-run a catch-up rebuild after Subscribe returns. Data-loss → in scope for the debut. *(PR #2062)*
