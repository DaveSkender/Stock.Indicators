...
You can also start a chain from a chosen price field with [`Use(CandlePart)`](/utilities/quotes/use-alternate-price):
...
- **Chainable inputs** — most indicators accept a reusable series, so they can take either raw `quotes` or another indicator's results.
...
- **[Batch (Series)](/guide/styles/batch#chaining-indicators)** — chain extension methods fluently (`quotes.ToObv().ToRsi(14)`). The standard, default approach.
...
- **[Stream hubs](/guide/styles/stream#chaining-indicators)** — subscribe one hub to another so chained indicators update automatically as new quotes stream in.

## See also

- [Indicator styles](/guide/styles/) — compare batch, buffer, and stream
- [Use alternate price](/utilities/quotes/use-alternate-price) — start a chain from a chosen price field
