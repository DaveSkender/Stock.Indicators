...
**Independent Test**: Can be fully tested by implementing a BufferList indicator (e.g., SMA buffer) and invoking `@buffer` for guidance. Success is measured by the agent correctly guiding interface selection, buffer utility usage, and ensuring proper inheritance from `BufferList<TResult>` and test base `BufferListTestBase`.

**Acceptance Scenarios**:

1. **Given** a developer needs to create a BufferList indicator, **When** they invoke `@buffer` with their indicator type, **Then** the agent recommends the correct interface (IIncrementFromChain, IIncrementFromQuote, or IIncrementFromPairs) based on the indicator's data requirements
2. **Given** a developer is implementing buffer management logic, **When** they ask `@buffer` for guidance, **Then** the agent directs them to use universal `BufferListUtilities` extension methods (`.Update()` or `.UpdateWithDequeue()`) instead of custom buffer logic
3. **Given** a developer needs to write BufferList tests, **When** they consult `@buffer`, **Then** the agent emphasizes inheriting from `BufferListTestBase` (not `TestBase`), implementing the correct test interface, and validating equivalence with Series results
4. **Given** a developer encounters a buffer state management issue, **When** they invoke `@buffer`, **Then** the agent explains proper `Clear()` implementation patterns, tuple usage for internal state, and avoidance of custom struct anti-patterns
5. **Given** a developer is unsure about constructor patterns, **When** they ask `@buffer`, **Then** the agent explains the two-constructor requirement (standard parameters, and parameters + quotes with chaining syntax `: this(params) => Add(quotes);`)
...
- **FR-001**: System MUST provide three custom agents: `@series`, `@buffer`, and `@streamhub` (updated)
...
**Initial Planning Clarifications** (November 3, 2025):
...
- Q: What naming pattern should be used for agent files? → A: ~~Simple style names matching agent invocation: `series.agent.md`, `buffer.agent.md`, `stream.agent.md`~~ **Corrected**: Align with instruction file naming pattern using `indicator-{style}` prefix: `indicator-series.agent.md`, `indicator-buffer.agent.md`, `indicator-stream.agent.md` (matches `.github/instructions/indicator-*.instructions.md` convention)

**Pre-Implementation Clarifications** (November 3, 2025 - before task generation):

- **Sub-agent Architecture Flexibility**: StreamHub sub-agents (@streamhub-state, @streamhub-performance, @streamhub-testing, @streamhub-pairs) are not exclusive to StreamHub - they can be shared with Series and Buffer agents if architecturally beneficial. The existing StreamHub agent implementation provides lessons learned and should be updated to align with the new agent pattern while maintaining backward compatibility with its specialized sub-agent ecosystem.

- **Naming Convention Alignment**: Agent files must follow the instruction file naming pattern for clarity and consistency: use `indicator-{style}` prefix (e.g., `indicator-series.agent.md`, `indicator-buffer.agent.md`, `indicator-stream.agent.md`). This aligns with the existing `.github/instructions/indicator-*.instructions.md` convention.

- **Official Documentation References**: All agent definitions must include reference links to official GitHub Copilot documentation:
  - Primary reference: [Custom agents configuration](https://docs.github.com/en/copilot/reference/custom-agents-configuration)
  - Supporting references: [Creating custom agents](https://docs.github.com/en/copilot/how-tos/use-copilot-agents/coding-agent/create-custom-agents), [About custom agents](https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-custom-agents)
  - Validation requirement: Use `#github_support_docs_search` and `#vscodeAPI` tools during implementation to ensure best practices alignment and API correctness
